...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_id", type=str, default="default", help="path to saving results"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="number of steps per one epoch",
    )
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), args.lr, betas=(0.9, 0.999), weight_decay=0
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch > 0) & (epoch % args.val_intervals == 0):
            val_loss, tp, fp, fn, tn, precision, accuracy = val(
