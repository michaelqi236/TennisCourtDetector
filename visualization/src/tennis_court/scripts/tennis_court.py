...
def draw_ground(all_markers, court_param):
    # Court ground
    ground_marker_0 = create_ground_marker(
        0,
        center=court_param.net_points[5],
        shape=[court_param.width, court_param.length],
        color=[0, 1, 0],
        z_offset=-0.01,
    )  # Green color
    buffer = 5
    ground_marker_1 = create_ground_marker(
        1,
        center=court_param.net_points[5],
        shape=[court_param.width + buffer, court_param.length + buffer],
        color=[0.9, 0.5, 0],
        z_offset=-0.03,
    )  # yellow color
...
def draw_net(all_markers, court_param):
    ...
    thickness = 0.02
    center = [
        court_param.net_points[5][0],
        court_param.net_points[5][1],
        court_param.net_height_at_center / 2,
    ]
    shape = [
        court_param.width + court_param.net_post_outside_court_distance * 2,
        thickness,
        court_param.net_height_at_center,
    ]
    ...
    line_points_1 = [
        Point(
            court_param.net_points[0][0],
            court_param.net_points[0][1],
            court_param.net_height_at_center,
        ),
        Point(
            court_param.net_points[2][0],
            court_param.net_points[2][1],
            court_param.net_height_at_center,
        ),
    ]
    net_marker_1 = create_line_marker(
        1, line_points_1, [1, 1, 1], name_space="tennis_court_net"
    )  # White color

    #  Poles
    diameter = 0.2
    net_marker_2 = create_cylinder_marker(
        2,
        court_param.net_points[1][:2],
        diameter,
        court_param.net_height_at_center,
        color=[0, 0, 0],
    )  # black color
    net_marker_3 = create_cylinder_marker(
        3,
        court_param.net_points[3][:2],
        diameter,
        court_param.net_height_at_center,
        color=[0, 0, 0],
    )  # black color
