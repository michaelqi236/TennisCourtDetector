...
class courtDataset(Dataset):
    ...
    def __init__(self, mode, input_height=720, input_width=1280, scale=2, hp_radius=55):
        ...
        with open(
            os.path.join(self.path_dataset, "data_{}.json".format(mode)), "r"
        ) as f:
    ...
    def filter_data(self):
        ...
        for i in range(len(self.data)):
            ...
            if (
                max_elems[0] < self.input_width
                and min_elems[0] > 0
                and max_elems[1] < self.input_height
                and min_elems[1] > 0
            ):
    ...
    def __getitem__(self, index):
        ...
        hm_hp = np.zeros(
            (self.num_joints + 1, self.output_height, self.output_width),
            dtype=np.float32,
        )
        ...
        for i in range(len(kps)):
            if (
                kps[i][0] >= 0
                and kps[i][0] <= self.input_width
                and kps[i][1] >= 0
                and kps[i][1] <= self.input_height
            ):
        ...
        draw_gaussian(
            hm_hp[self.num_joints],
            (int(x_ct / self.scale), int(y_ct / self.scale)),
            self.hp_radius,
        )
